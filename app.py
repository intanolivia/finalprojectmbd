...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS PATIENT_DATA (id serial, doctor_name varchar, patient_name varchar, gender char(25), \
                                                       symptom text, handphone varchar, address text, tanggal date);')
    session.execute(query)

st.header('PATIENT DATA MANAGEMENT SYSTEM')
page = st.sidebar.selectbox("Choose Menu", ["View Data", "Edit Data"])

if page == "View Data":
    data = conn.query('SELECT * FROM patient_data ORDER BY id;', ttl="0").set_index('id')
    st.dataframe(data)

if page == "Edit Data":
    if st.button('Add Patient Data'):
        with conn.session as session:
            query = text('INSERT INTO patient_data (doctor_name, patient_name, gender, symptom, handphone, address, waktu, tanggal) \
                          VALUES (:1, :2, :3, :4, :5, :6, :7, :8);')
            session.execute(query, {'1':'', '2':'', '3':'', '4':'[]', '5':'', '6':'', '7':None, '8':None})
            session.commit()

    data = conn.query('SELECT * FROM patient_data ORDER BY id;', ttl="0")
            ...
            with st.form(f'data-{id}'):
                ...
                with col1:
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            query = text('UPDATE patient_data \
                ...
                with col2:
                    if st.form_submit_button('DELETE'):
                        query = text(f'DELETE FROM patient_data WHERE id=:1;')
