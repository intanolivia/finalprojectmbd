...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS CINEMA_SCHEDULE (id serial, movie_title varchar, genre varchar, director varchar, \
...
if page == "View Schedule":
    data = conn.query('SELECT * FROM cinema_schedule ORDER BY id;', ttl="0").set_index('id')
    st.dataframe(data)

if page == "Edit Schedule":
    if st.button('Add Movie Schedule'):
        with conn.session as session:
            query = text('INSERT INTO cinema_schedule (movie_title, genre, director, release_date, start_time, end_time, \
                                                      theater_number, ticket_price) \
                          VALUES (:1, :2, :3, :4, :5, :6, :7, :8);')
            session.execute(query, {'1':'', '2':'', '3':'', '4':None, '5':None, '6':None, '7':None, '8':None})
            session.commit()

    data = conn.query('SELECT * FROM cinema_schedule ORDER BY id;', ttl="0")
    for _, result in data.iterrows():        
        id = result['id']
        movie_title_lama = result["movie_title"]
        genre_lama = result["genre"]
        director_lama = result["director"]
            ...
            with st.form(f'movie-data-{id}'):
                ...
                with col1:
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            query = text('UPDATE cinema_schedule \
                ...
                with col2:
                    if st.form_submit_button('DELETE'):
                        query = text(f'DELETE FROM cinema_schedule WHERE id=:1;')
