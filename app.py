    ...
    for _, result in data.iterrows():
        ...
        with st.expander(f'{movie_title_lama}'):
            with st.form(f'movie-data-{id}'):
                ...
                genre_baru = st.selectbox("Genre", list_genre, list_genre.index(genre_lama) if genre_lama in list_genre else 0)
                director_baru = st.text_input("Director", director_name_lama)
                release_date_baru = st.date_input("Release Date", datetime.strptime(release_date_lama, "%Y-%m-%d") if release_date_lama else None)
                start_time_baru = st.time_input("Start Time", datetime.strptime(start_time_lama, "%H:%M:%S").time() if start_time_lama else None)
                end_time_baru = st.time_input("End Time", datetime.strptime(end_time_lama, "%H:%M:%S").time() if end_time_lama else None)
                theater_number_baru = st.selectbox("Theater Number", list_theater_number, list_theater_number.index(str(theater_number_lama)) if theater_number_lama in list_theater_number else 0)
