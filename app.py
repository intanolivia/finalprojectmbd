...
if page_cinema == "View Cinema Schedule":
    data_cinema = conn.query('SELECT * FROM movie_schedule ORDER By id;', ttl="0").set_index('id')
    st.dataframe(data_cinema)

if page_cinema == "Edit Cinema Schedule":
    if st.button('Add Movie Schedule'):
        with conn.session as session:
            query_add_cinema = text('INSERT INTO movie_schedule (movie_title, genre, director, release_date, start_time, end_time, \
